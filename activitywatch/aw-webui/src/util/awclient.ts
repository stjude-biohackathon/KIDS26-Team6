...
export function createClient(force?: boolean): AWClient {
  ...
  // Upstream default points at aw-server's --testing instance (port 5666).
  // We point our fork at the real aw-server instead, since we want this
  // dashboard to show our actual data, not an empty testing database.
  // (Overriding via the AW_SERVER_URL env var/DefinePlugin trips a build
  // error in this project's current ts-loader/babel-loader combo, so this
  // is hardcoded directly instead of relying on that env var.)
  if (!production) {
