...
**The real key, once obtained, still failed with "Anthropic API error: Connection error" — root cause was an environment bug, not the key, network, or our code.** Traced by reproducing the exact `client.messages.create()` call in isolation (bypassing Flask) to see the real traceback instead of the SDK's generic wrapper: the installed `brotli` package (1.0.9, from this Anaconda environment) has a `Decompressor.process()` that doesn't accept the `output_buffer_limit` keyword argument the installed `httpx2` version calls it with — `TypeError: process() takes no keyword arguments`, raised while decoding the (successfully received!) HTTP response body, then re-wrapped by the Anthropic SDK as a generic `APIConnectionError`. A raw `curl` to the same endpoint worked fine the whole time (got a real 401), which is what pointed away from network/DNS/proxy causes and toward something client-library-specific. Fixed with `pip install --upgrade Brotli` (1.0.9 → 1.2.0) — confirmed the exact same isolated repro then returned a real `"OK"` response from Claude. Anyone else setting this up on a different machine with an old `brotli` install could hit the identical thing — if `draft_skill` fails with "Connection error" despite a correct, valid key, check this first before suspecting the key or network.

**Then confirmed the whole feature genuinely works, real key + real data, both ways**: curl first (real merge, real Claude response, real file written to `drafted_skills/`), then a completely fresh real browser click on "Merge & Draft Skill" — rendered a real drafted `SKILL.md` in the UI, and (worth noting) Claude's draft was self-aware enough to correctly infer it was watching its own debugging session unfold in the OCR text, and explicitly flagged its own data limitations rather than inventing detail it didn't have.
