#!/usr/bin/env python3
"""Create a bounded, non-executing inventory of skill-forge evidence sources."""
    ...
    for source in requested:
        ...
        for candidate in candidates:
            if candidate.is_symlink():
                records.append(
                    {"path": str(candidate), "status": "skipped_symlink"}
                )
                continue
...
def normalizeText(path: Path, maxBytes: int) -> tuple[str | None, list[str]]:
    ...
    if size > maxBytes:
        return None, [
            f"Extraction skipped: {size} bytes exceeds --maxBytesPerFile={maxBytes}."
        ]
