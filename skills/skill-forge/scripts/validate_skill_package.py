...
def validateRuntimeContract(
    ...
    if runtimeReferencePath.is_file():
        ...
        for token in (
            ...
            if token not in runtimeReference:
                errors.append(
                    "Runtime reproducibility reference omits {!r}.".format(token)
                )
    ...
    if summaryTemplatePath.is_file():
        ...
        if summaryTemplate is not None:
            missingSummaryFields = sorted(
                requiredSummaryFields - set(summaryTemplate)
            )
            if missingSummaryFields:
                errors.append(
                    "Run summary template omits fields: "
                    + ", ".join(missingSummaryFields)
                )
    ...
    for instruction in requiredInstructions:
        if instruction.lower() not in body.lower():
            errors.append(
                f"SKILL.md runtime contract does not mention {instruction!r}."
            )
    if "display" not in body.lower() or "chat" not in body.lower():
        errors.append(
            "SKILL.md must require displaying the finalized findings summary in chat."
        )
    ...
    if not isinstance(package.get("commandExecutionExpected"), bool):
        errors.append(
            "skill-package.json commandExecutionExpected must be boolean."
        )
    ...
    if not isinstance(declaredArtifacts, list) or not all(
        ...
        errors.append(
            "skill-package.json requiredRunArtifacts must be a string array."
        )
    else:
        missingArtifacts = sorted(REQUIRED_RUN_ARTIFACTS - set(declaredArtifacts))
        if missingArtifacts:
            errors.append(
                "skill-package.json omits required run artifacts: "
    ...
    if primary is not None:
        ...
        if not primaryPath.is_file() or not primaryPath.read_text(
            encoding="utf-8"
        ).strip():
            ...
            if isinstance(values, list):
                for value in values:
                    if (
                        ...
                        and json.dumps(value) not in environmentText
                    ):
                        errors.append(
                            f"environment.yml omits runtimeEnvironment {field} "
                            f"value {value!r}."
                        )
    if manager == "venv" and (root / "requirements.txt").is_file():
        requirementLines = [
            value.strip()
            for value in (root / "requirements.txt")
        ...
        if requirementLines != environment.get("pipDependencies"):
            errors.append(
                "requirements.txt does not match runtimeEnvironment pipDependencies."
            )
    ...
    if not isinstance(externalArtifacts, list):
        ...
        if not externalPath.is_file() or not externalPath.read_text(
            encoding="utf-8"
        ).strip():
            errors.append(
                "Runtime externalArtifacts require non-empty external-artifacts.txt."
            )
        else:
            externalLines = [
                value.strip()
            ...
            if externalLines != externalArtifacts:
                errors.append(
                    "external-artifacts.txt does not match runtimeEnvironment."
                )
    pythonConstraint = environment.get("python")
    if not isinstance(pythonConstraint, str) or not pythonConstraint.strip():
        errors.append(
            "runtimeEnvironment must declare Python for scripts/record_run.py."
        )
    if manager in {"venv", "none", "system", "codebase"} and not (
        root / "python-requirement.txt"
    ).is_file():
        errors.append(
            f"Runtime manager {manager!r} requires python-requirement.txt."
        )
    elif manager in {"venv", "none", "system", "codebase"}:
        pythonFileValue = (root / "python-requirement.txt").read_text(
            encoding="utf-8"
        ).strip()
        if pythonFileValue != pythonConstraint:
            errors.append(
                "python-requirement.txt does not match runtimeEnvironment."
            )
    if manager == "codebase":
        if packaging != "cbd":
        ...
        if not isinstance(codebaseFile, str) or not codebaseFile.strip():
            errors.append(
                "Codebase runtime requires codebaseEnvironmentFile in package metadata."
            )
    if manager == "container":
        ...
        if imagePath.is_file() and imagePath.read_text(
            encoding="utf-8"
        ).strip() != image:
            errors.append(
                "container-image.txt does not match runtimeEnvironment."
            )
    if environment.get("systemDependencies"):
        systemPath = root / "system-requirements.txt"
        if not systemPath.is_file():
            errors.append(
                "systemDependencies require system-requirements.txt."
            )
        else:
            systemLines = [
                value.strip()
            ...
            if systemLines != environment.get("systemDependencies"):
                errors.append(
                    "system-requirements.txt does not match runtimeEnvironment."
                )
        ...
        if not isinstance(dependencies, list):
            ...
            for index, dependency in enumerate(dependencies):
                if (
                    ...
                    not in {"public_tool", "public_pipeline"}
                ...
                if dependency.get("kind") == "public_pipeline" and not (
                    ...
                    errors.append(
                        "Required public pipeline at index {} has no version "
                        "constraint.".format(index)
                    )
                ...
                matchingSpecs = availableMap.get(
                    normalizedPackageName(environmentPackage), []
                )
                ...
                if (
                    ...
                    errors.append(
                        ...
                        "constraint absent from its environment declaration.".format(
                            index
                        )
                    )
    if environment.get("lockStrategy") == "direct-pins":
        ...
        if unpinned:
            errors.append(
                "Runtime direct-pins lacks constraints for: " + ", ".join(unpinned)
            )
        if isinstance(pythonConstraint, str) and not hasVersionConstraint(
            pythonConstraint
        ):
            errors.append(
                "Runtime direct-pins requires a constrained Python version."
            )
...
def validatePackage(
    ...
    if generatedBy == "skill-forge":
        if declaredPackaging not in {"cbd", "std"}:
            errors.append(
                "Generated skill metadata.packaging must be CBD or STD."
            )
        else:
            if detectedPackaging is None:
                ...
                errors.append(
                    "Detected packaging disagrees with metadata.packaging."
                )
            if not name.endswith(f"-{declaredPackaging}"):
                errors.append(
                    f"Generated skill name must end in '-{declaredPackaging}'."
                )
