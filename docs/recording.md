...
AutoCAB measures complete-span recall, records with zero missed spans, missed
spans per 1,000 records, and over-redaction on a committed synthetic corpus.
The generated
[PHI redaction benchmarking report](phi-redaction-benchmarking.md) is the source
of truth for current values, model comparisons, and limitations.

These detector measurements do not establish performance on external clinical
text or constitute a HIPAA Safe Harbor determination. They also do not cover
full-face photographs or identifiers that exist only inside images.
