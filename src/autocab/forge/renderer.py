#!/usr/bin/env python3
"""Validate a SkillSpec and deterministically render a staged skill package."""
...
def assetsDir() -> Path:
    """Locate forge templates in a source checkout or installed wheel."""

    candidates = (
        Path(__file__).resolve().parent / "assets",
        Path(sys.prefix) / "share" / "autocab" / "skill-forge",
        Path(__file__).resolve().parents[3] / "skills" / "skill-forge" / "assets",
    )
    for candidate in candidates:
        if (candidate / "std-skill.template.md").is_file():
            return candidate
    raise ForgeError("Packaged Skill Forge templates are missing.")
...
def ensureEmptyRunDir(path: Path, skillRoot: Path) -> None:
    ...
    try:
        ...
        raise ForgeError(f"Run output must be outside the skill-forge source package: {resolved}")
    except ValueError:
        pass
    if resolved.exists():
        ...
        if any(resolved.iterdir()):
            raise ForgeError(f"--outputDir must be empty; refusing to overwrite: {resolved}")
    else:
...
def renderDependencies(dependencies: list[dict[str, Any]]) -> str:
    ...
    for dependency in dependencies:
        ...
        if dependency["environmentPackage"]:
            details.append(f"environment package `{dependency['environmentPackage']}`")
...
def renderCodebaseRequirements(codebase: dict[str, Any]) -> str:
    ...
    for index, root in enumerate(roots, start=1):
        ...
        remote = f"; expected local origin `{root['gitRemote']}`" if root.get("gitRemote") else ""
...
def renderRuntimeEnvironment(spec: dict[str, Any]) -> str:
    ...
    if environment["externalArtifacts"]:
        lines.append("- Versioned external workflows/data: `external-artifacts.txt`.")
    if manager == "conda":
        lines.extend(
            [
...
def fillSkillTemplate(spec: dict[str, Any], assetsDir: Path) -> str:
    ...
    templateName = (
        "cbd-skill.template.md" if spec["packaging"] == "cbd" else "std-skill.template.md"
    )
...
def evidenceSummary(spec: dict[str, Any]) -> str:
    ...
    confidenceText = ", ".join(f"{key}={confidence[key]}" for key in sorted(confidence)) or "none"
...
def renderReview(
    ...
    """Fill REVIEW.md."""
    template = Template((assetsDir / "review.template.md").read_text(encoding="utf-8"))
    return (
        template.substitute(
            {
                "name": spec["name"],
                ...
                "manual_blocked_steps": selectedSteps(spec, {"manual", "blocked"}),
...
def renderEvaluationPrompts(spec: dict[str, Any], assetsDir: Path) -> str:
    ...
    template = Template((assetsDir / "evaluation-prompts.template.md").read_text(encoding="utf-8"))
    return (
        template.substitute(
            {
                "name": spec["name"],
...
def renderCondaEnvironment(spec: dict[str, Any]) -> str:
    ...
    if environment["pipDependencies"]:
        if not any(re.match(r"^pip(?:$|[<>=!~ ])", value) for value in dependencies):
            lines.append('  - "pip"')
        lines.append("  - pip:")
        lines.extend(f"      - {json.dumps(value)}" for value in environment["pipDependencies"])
    return "\n".join(lines) + "\n"
...
def writeRuntimePackageFiles(
    ...
    shutil.copy2(assets / "record_run.py", recorderTarget)
    recorderTarget.chmod(0o755)
    runtimeReference = (
        (assets / "runtime-reproducibility.template.md")
    ...
    shutil.copy2(
        assets / "run-summary.template.json",
    ...
    manager = environment["manager"]
    if manager == "conda":
        (proposalDir / "environment.yml").write_text(renderCondaEnvironment(spec), encoding="utf-8")
    elif manager == "venv":
    ...
    packageMetadata = {
        ...
        "dependencies": [
            {key: value for key, value in dependency.items() if key != "sourcePath"}
            for dependency in spec["dependencies"]
        ],
    ...
    return [
        ...
        },
        {
            ...
            "sha256": fileSha256(referencesDir / "runtime-reproducibility.md"),
        },
        {
...
def prepareProposal(
    ...
    (proposalDir / "SKILL.md").write_text(fillSkillTemplate(spec, assets), encoding="utf-8")
    if spec["operation"] == "create" or not (proposalDir / "README.md").exists():
        (proposalDir / "README.md").write_text(generatedReadme(spec), encoding="utf-8")
    if spec["operation"] == "create" or not (proposalDir / "CHANGELOG.md").exists():
        (proposalDir / "CHANGELOG.md").write_text(generatedChangelog(spec), encoding="utf-8")
    else:
    ...
    (examplesDir / evaluationName).write_text(
        renderEvaluationPrompts(spec, assets), encoding="utf-8"
    )

    scriptsDir = proposalDir / "scripts"
    copied = writeRuntimePackageFiles(spec, proposalDir, assets)
    if spec["packaging"] == "cbd":
        scriptsDir.mkdir(parents=True, exist_ok=True)
        configSource = Path(__file__).with_name("cbd_config.py")
    ...
    if args.allowCodeCopy:
        for dependency in spec["dependencies"]:
            ...
            source = Path(sourceValue).expanduser()
            if not source.is_file() or source.is_symlink():
                raise ForgeError(f"Custom dependency is not a regular non-symlink file: {source}")
...
def parseArgs(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--agentWorkflowFile", help="Caller-prepared workflow/provenance notes.")
...
def runForge(args: argparse.Namespace) -> int:
    ...
    skillRoot = Path(__file__).resolve().parent
    assets = assetsDir()
    ...
    review = renderReview(spec, assets, proposalRendered, args.allowCodeCopy)
    ...
    metadata = {
        ...
        "runId": runId,
        "timestampUtc": utcNow(),
        "commandRedacted": ("python scripts/forge_skill.py --spec <SPEC> --outputDir <RUN_DIR>"),
        "parameters": {
            "operation": spec["operation"],
            "proposedSkillVersion": spec["skillVersion"],
            "packaging": spec["packaging"],
