#!/usr/bin/env python3
"""Resolve, validate, and atomically update AutoCAB CBD configuration."""
...
def resolveConfigPath(
    ...
    if len(existing) > 1:
        ...
        raise ConfigError(
            f"Multiple AutoCAB CBD configs exist; pass --config to select one: {rendered}"
        )
...
def validateSentinel(value: str) -> str:
    ...
    if path.is_absolute() or ".." in path.parts or value.strip() in {"", "."}:
        raise ConfigError(f"Sentinel must be a non-empty relative path without '..': {value!r}")
...
def validateConfigData(data: Any) -> dict[str, Any]:
    ...
    for skill, entry in skills.items():
        ...
        if not isinstance(gitRemotes, dict) or not all(
            isinstance(key, str) and isinstance(value, str) for key, value in gitRemotes.items()
        ):
...
def loadConfig(path: Path, allowMissing: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConfigError(f"Invalid CBD config JSON at line {exc.lineno}: {exc.msg}") from exc
...
def parseGitRemote(values: list[str], roots: list[str]) -> dict[str, str]:
    ...
    for value in values:
        ...
        if normalizedRoot not in rootSet:
            raise ConfigError(f"--gitRemote root is not present in --root values: {rootValue}")
        if not remote:
...
def validateSkillEntry(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    for sentinelValue in entry["sentinels"]:
        ...
        sentinelReports.append({"sentinel": sentinel, "matches": matches, "valid": bool(matches)})
...
def parseArgs(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    setParser = subparsers.add_parser("set", help="Create or replace one skill entry atomically.")
...
def runCommand(args: argparse.Namespace) -> int:
            ...
            json.dumps(
                {
                    ...
                    "agents": {name: str(relative) for name, relative in AGENT_PATHS.items()},
                    "compatibilityPaths": [str(Path(".codex/autoCAB") / CONFIG_FILENAME)],
                },
                indent=2,
            )
        )
    ...
    if args.command == "get":
        ...
        return 0
    if args.command == "set":
        roots = list(dict.fromkeys(str(Path(value).expanduser().resolve()) for value in args.root))
        sentinels = list(dict.fromkeys(validateSentinel(value) for value in args.sentinel))
        skills[args.skill] = {
            "roots": roots,
            "sentinels": sentinels,
