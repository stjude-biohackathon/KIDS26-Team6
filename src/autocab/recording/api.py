...
def create_app(recorder: Recorder, token: str) -> FastAPI:
    ...
    def ui_asset(filename: str, media_type: str) -> Response:
        content = resources.files("autocab.ui").joinpath(filename).read_text(encoding="utf-8")
        return Response(
            content=content,
    ...
    def index() -> str:
        html = resources.files("autocab.ui").joinpath("index.html").read_text(encoding="utf-8")
