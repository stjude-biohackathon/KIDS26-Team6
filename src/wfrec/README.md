...
in the design plan. Everything is local-only with no outbound network requests by default;
the control API binds to loopback behind a token unless you opt in with
``--allow-remote`` / ``WFREC_ALLOW_REMOTE=1``.
