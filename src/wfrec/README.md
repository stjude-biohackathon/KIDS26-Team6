...
accession, account, device, health-plan id and licence, **0.88 overall** across
...
tested floor rather than a footnote. A pattern tier does not detect names; the
GLiNER tier that closes that gap is specified and not yet built. Read
...
**The default is local, with no egress.** The regex tier and the GLiNER tier are
local by construction — they are libraries running in this process, and no
configuration makes them reach the network. An LLM tier pointed at a loopback
