...
from autocab.recording.events import SHELL_COMMAND
from autocab.recording.recorder import Recorder
from autocab.recording.session import Session, SessionStore
...
def test_missing_devsql_keeps_hook_spool_active(autocab_home: Path) -> None:
...
def test_no_daemon_forces_hook_spool(autocab_home: Path) -> None:
    recorder = Recorder(
        supervise=False,
...
def test_resume_reselects_backend(autocab_home: Path) -> None:
