...
from autocab.recording.state import (
    RecorderState,
...
def test_sentinel_absent_until_a_session_is_active(autocab_home):
...
def test_stopping_removes_the_sentinel(autocab_home):
...
def test_flags_reflect_each_source(autocab_home):
    state = RecorderState.load()
    state.active_session = "s1"
...
def test_legacy_shell_output_does_not_change_hook_flags(autocab_home):
    state = RecorderState.load()
    state.active_session = "s1"
...
def test_devsql_backend_suppresses_only_the_shell_hook(autocab_home):
    state = RecorderState.load()
    state.active_session = "s1"
...
def test_trashed_sessions_round_trip_through_state(autocab_home):
    state = RecorderState.load()
...
def test_default_analyst_round_trips_through_state(autocab_home):
    state = RecorderState.load()
...
def test_default_analyst_falls_back_to_os_username(autocab_home, monkeypatch):
    monkeypatch.setattr(paths.getpass, "getuser", lambda: "system-user")

    assert resolved_default_analyst(RecorderState()) == "system-user"


def test_sentinel_is_a_single_tab_separated_line(autocab_home):
...
def test_corrupt_state_file_yields_defaults_not_an_exception(autocab_home):
...
def test_transaction_persists_and_refreshes_sentinel(autocab_home):
...
def test_runtime_dir_is_not_inside_home(autocab_home, monkeypatch):
    ...
    monkeypatch.setenv("XDG_RUNTIME_DIR", str(autocab_home.parent / "xdg"))
    (autocab_home.parent / "xdg").mkdir(exist_ok=True)
