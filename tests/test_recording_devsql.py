...
from autocab.recording.devsql import (
    DevSQLClient,
...
def test_discover_finds_devsql(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("autocab.recording.devsql.shutil.which", lambda name: "/opt/bin/devsql")
...
def test_discover_reports_missing_devsql(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("autocab.recording.devsql.shutil.which", lambda name: None)
